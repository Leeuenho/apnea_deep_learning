# -*- coding: utf-8 -*-
"""
Created on Wed Sep 29 16:37:54 2021
...
fn_list = []
file_name = []
path = 'D:/apnea-ecg-database-1.0.0 (1)/apnea-ecg-database-1.0.0//'
file_list = os.listdir(path)
file_list_apn= [file for file in file_list if file.endswith('.apn')]
file_list_apnea = [ff for ff in file_list_apn if ff.startswith(('a', 'b', 'c'))]

Total_Xout = pd.DataFrame()

for i in range(len(file_list_apn)):
    if len(file_list_apn[i])==7:
        fn_list.append(file_list_apn[i])
...
for k in range(len(fn_list)):
    ...
    for i in range(annotation.ann_len):   
        ...
        ***노이즈 제거
        ...
        rri_by_min.insert(rri_by_min.shape[1],str(file_name[k])+'xPos_'+str(int(start/100)),ywin) #dataFrame.insert(data 추가할 위치, 추가할 열의 이름, 추가할 data)
...
x_train = Total_Xout.transpose()  

x_train.to_csv('wfdb_filteredData.csv')
